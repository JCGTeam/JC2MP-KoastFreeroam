...
function StateRacing:PostTick(args)
    ...
    if self.sendCheckpointTimer:GetSeconds() >= settings.sendCheckpointDistanceInterval then
        ...
        Network:Send("ReceiveCheckpointDistanceSqr", {LocalPlayer:GetId(), self:GetTargetCheckpointDistanceSqr(), self.targetCheckpoint})
    end
    -- Manage the exit vehicle coroutine.
    if self.race.assignedVehicleId == -1 or LocalPlayer:InVehicle() or LocalPlayer:GetHealth() == 0 then
        self.exitVehicleCoroutine = nil
