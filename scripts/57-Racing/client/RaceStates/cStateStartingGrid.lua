...
function StateStartingGrid:__init(race, args)
    ...
    local maxPresentTime = (self.race.startingGridSeconds - settings.countDownInterval * settings.countDownNumMessages)
...
function StateStartingGrid:Render()
    ...
    if #self.countDownTimes > 0 and timerSeconds > self.race.startingGridSeconds - self.countDownTimes[1] then
        LargeMessage(tostring(settings.countDownNumMessages - self.messageCount), settings.countDownInterval * 1)
