...
function StateVehicleSelection:__init(race, args)
...
function StateVehicleSelection:CreateMenus()
    ...
    for index, vehicleInfo in ipairs(self.vehicles) do
        ...
        label:SetMargin(Vector2(0, 6), Vector2.Zero)
    ...
    self.colorPicker:SetMargin(Vector2(0, 6), Vector2.Zero)
    ...
    for index, vehicleUsage in ipairs(vehicleUsages) do
        for index, vehicleInfo in ipairs(self.vehicles) do
            if vehicleInfo.modelId == vehicleUsage.modelId then
                ...
                vehicleInfo.usageLabel:SetText(string.format("%i/%i", vehicleInfo.used, vehicleInfo.available))
