...
function RaceModules.Event:__init()
...
function RaceModules.Event:RaceOrSpectateEnd()
    if self.endRaceButton then self.endRaceButton:Remove() end
...
function RaceModules.Event:RaceFinish()
    if self.isOwner then
        RaceBase.instance:Message(string.format("Open the race menu (/%s) to end the race.", settings.command))
    end
end
