...
function Icon:Render()
    if Game:GetState() ~= GUIState.Game or self.isVisible == false then return end
    ...
    if self.type == Icon.Type.Enabled then
        ...
        DrawModels()
        if Icon.frameEnabledModel then
            Icon.frameEnabledModel:Draw()
        end
    elseif self.type == Icon.Type.Disabled then
        ...
        Render:FillArea(Vector2(0.05, 0.05), Vector2(0.9, 0.9), color)
        DrawModels()
        if Icon.frameDisabledModel then
            Icon.frameDisabledModel:Draw()
        end
    end
