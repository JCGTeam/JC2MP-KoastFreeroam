...
function CoursesTab:__init(...)
    TabBase.__init(self, "Карты", ...)
...
function CoursesTab:CourseSelected()
    ...
    if self.coursesList:GetDataBool("isValid") == false then return end
...
function CoursesTab:RecordButtonPressed(button)
    ...
    RaceMenu.instance:AddRequest("RequestCourseRecords", {self.selectedCourseInfo[1], self.recordsIndex})
    self:SetRecordButtonsEnabled(false)
end
