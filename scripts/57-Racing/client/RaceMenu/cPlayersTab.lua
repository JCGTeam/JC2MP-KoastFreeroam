...
function PlayersTab:__init(...)
    TabBase.__init(self, "Игроки", ...)
...
function PlayersTab:Search()
    ...
    if self.sortType == PlayerSortType.None then return end
...
function PlayersTab:ReceiveSortedPlayers(args)
    ...
    for index, sortedPlayer in ipairs(sortedPlayers) do
        ...
        if playerSortType == PlayerSortType.PlayTime then
            local text = string.format("%iч, %iм, %iс", Utility.SplitSeconds(sortedPlayer[3]))
