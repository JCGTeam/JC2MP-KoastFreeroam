...
function Spectate:__init(args)
    RaceBase.__init(self, args)
    ...
    self.target = {id = -1, position = args.position or Vector3(0, 5000, 0)}
...
function Spectate:RenderRacing()
    ...
    if Game:GetState() == GUIState.Game then
        ...
        RaceGUI.DrawTimers {recordTime = self.recordTime, recordTimePlayerName = self.recordTimePlayerName, courseType = self.course.type}
        RaceGUI.DrawRaceProgress {currentCheckpoint = self.leaderboard[1].checkpointsHit or 0, checkpointCount = self.totalCheckpointCount}
        RaceGUI.DrawLeaderboard {leaderboard = self.leaderboard, playerIdToInfo = self.playerIdToInfo}
        RaceGUI.DrawPositionTags {leaderboard = self.leaderboard}
    end
end
...
function Spectate:RequestTarget()
    ...
    if self.targetRequest ~= nil and self.targetRequest.timer:GetSeconds() < settings.spectatorRequestInterval then return end
