...
function RaceGUI.DrawCourseName(courseName)
    local textHeight = Render:GetTextHeight("|", TextSize.Default)
    DrawText(Vector2(0.5 * Render.Width, textHeight * 3.5 + 1), courseName, settings.textColor2, TextSize.Default, "center")
end
...
function RaceGUI.DrawTargetArrow(args)
    ...
    local shouldDraw = (args.targetArrowValue == nil or args.numTicks == nil or args.targetArrowValue == maxValue or math.floor(args.numTicks / settings.targetArrowFlashInterval) % 2 == 0)
...
function RaceGUI.DrawLapCounter(args)
    ...
    if args.isFinished then count = total end

    -- "Lap/Checkpoint" label
    DrawText(NormVector2(settings.lapLabelPos.x, settings.lapLabelPos.y), label, settings.textColor, settings.lapLabelSize, "center")
    -- Counter (ie "1/3")
    DrawText(NormVector2(settings.lapCounterPos.x, settings.lapCounterPos.y), string.format("%i/%i", count, total), settings.textColor2, settings.lapCounterSize, "center")
end
...
function RaceGUI.DrawTimers(args)
    ...
    local leftX = (settings.timerLabelsStart.x - (Render:GetTextWidth("-00:00:00", settings.timerLabelsSize) / Render.Width) * 2)

    local AddLine = function(label, value)
        DrawText(NormVector2(leftX, currentY), label, settings.textColor, settings.timerLabelsSize, "right")
        DrawText(NormVector2(settings.timerLabelsStart.x, currentY), value, settings.textColor2, settings.timerLabelsSize, "right")
...
function RaceGUI.DrawLeaderboard(args)
    ...
    for n = 1, math.min(#args.leaderboard, settings.leaderboardMaxPlayers) do
        ...
        DrawText(currentPos + Vector2(charHeight * 4, 0), Utility.NumberToPlaceString(n), settings.textColor, settings.leaderboardTextSize, "right")
        currentX = currentX + charWidth * 2.25
        DrawText(currentPos + Vector2(charHeight * 1, 0), string.format("%s", playerName), playerInfo.color, settings.leaderboardTextSize, "right")
        currentX = currentX + playerNameWidth
        -- If this is us, point out our name with arrows.
        if playerId == LocalPlayer:GetId() then
            DrawText(currentPos + Vector2(charHeight * 2, 0), "<", settings.textColor, settings.leaderboardTextSize, "right")
        end
...
function RaceGUI.DrawPositionTag(playerId, position)
    ...
    local worldPos = player:GetBonePosition("ragdoll_Head") + (player:GetAngle() * Vector3(0, 0.68, 0))
    ...
    DrawText(screenPos, Utility.NumberToPlaceString(position), player:GetColor(), size, "center", scale)
end
...
function RaceGUI.DrawNextCheckpointArrow(args)
    ...
    if args.targetCheckpoint == #args.checkpoints then
        if args.courseType == "Linear" then
            return
        elseif args.courseType == "Circuit" and args.numLaps == args.currentLap then
            return
        end
    end
    ...
    -- Check if target checkpoint is the start/finish.
    if args.courseType == "Circuit" and args.targetCheckpoint == #args.checkpoints then
    ...
    local angle = Angle.FromVectors(Vector3(0, 0, -1), (cpNext - cpTarget):Normalized())
    angle.roll = 0
    ...
    local dotMod = Vector3.Dot((cpTarget - Camera:GetPosition()):Normalized(), (cpNext - cpTarget):Normalized())
    ...
    for n = 1, #triangles do
        Render:FillTriangle(angle * triangles[n][1] + cpTarget, angle * triangles[n][2] + cpTarget, angle * triangles[n][3] + cpTarget, color)
    end
end
