...
function RaceBase:UpdateLeaderboard(racePosInfo)
    ...
    for cp, array in pairs(racePosTracker) do
        ...
        table.sort(racePosTrackerArray[cp], function(id1, id2) return (playerIdToCheckpointDistanceSqr[id1] < playerIdToCheckpointDistanceSqr[id2]) end)
    end
    ...
    for n = 1, #finishedPlayerIds do
        local entry = {playerId = finishedPlayerIds[n], isFinished = true, checkpointsHit = self.totalCheckpointCount}
    ...
    for cpIndex = currentCheckpoint, 0, -1 do
        ...
        for playerIdIndex = 1, numPlayerIds do
            local entry = {playerId = racePosTrackerArray[cpIndex][playerIdIndex], isFinished = false, checkpointsHit = cpIndex}
            table.insert(self.leaderboard, entry)
            self.numPlayers = self.numPlayers + 1
        end
    end
