...
Stats.UpdateFromOldVersion = function(version)
    ...
    local tableNames = {"RacePlayers", "RaceResults", "RaceCourses", "RaceCourseVotes", "RaceVersion"}
    ...
    for index, raceResult in ipairs(database.RaceResults) do
        local command = SQL:Command("insert into RaceResults(SteamId , Place , CourseFileNameHash , Vehicle , BestTime) " .. "values(?,?,?,?,?)")
    ...
    for index, raceCourse in ipairs(database.RaceCourses) do
        local command = SQL:Command("insert into RaceCourses values(?,?,?)")
    ...
    for index, raceCourseVote in ipairs(database.RaceCourseVotes) do
        local command = SQL:Command("insert into RaceCourseVotes values(?,?,?)")
