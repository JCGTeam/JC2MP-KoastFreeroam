...
function Spectator:__init(race, player)
    ...
    local args = {className = "Spectate", raceInfo = self.race.info, stateName = self.race.stateName}
...
function Spectator:RequestTarget(playerId)
    if self.requestTimer == nil or self.requestTimer:GetSeconds() > settings.spectatorRequestInterval * 1.2 + 0.2 then
