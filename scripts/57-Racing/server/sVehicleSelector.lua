...
function VehicleSelector:__init(state, racer)
...
function VehicleSelector:SpawnVehicle()
    ...
    if self.vehicle then self.vehicle:Remove() end

    local vehicleInfo = self.state.vehicles[self.vehicleIndex]

    -- Spawn the vehicle, unless they chose on-foot.
    if vehicleInfo.modelId == -1 then self.vehicle = nil return end
...
function VehicleSelector:VehicleSelectionLoaded(unused, player)
    ...
    if player ~= self.racer.player then return end
    ...
    for index, vehicleInfo in ipairs(self.state.vehicles) do
        table.insert(vehicleUsages, {
            modelId = vehicleInfo.modelId,
            count = vehicleInfo.used
        })
    end
...
function VehicleSelector:VehicleSelected(vehicleIndex, player)
    ...
    if type(vehicleIndex) ~= "number" or vehicleIndex < 1 or vehicleIndex > #self.state.vehicles then return end

    local vehicleInfo = self.state.vehicles[vehicleIndex]
    if vehicleInfo.used == vehicleInfo.available then return end
    ...
    local vehicleUsages = {{modelId = oldVehicleInfo.modelId, count = oldVehicleInfo.used}, {modelId = vehicleInfo.modelId, count = vehicleInfo.used}}
...
function VehicleSelector:VehicleTemplateSelected(templateIndex, player)
    ...
    -- Check client arguments.
    if type(templateIndex) ~= "number" or templateIndex < 1 or templateIndex >
        #self.state.vehicles[self.vehicleIndex].templates then
        return
    end
...
function VehicleSelector:VehicleSetColors(colors, player)
    ...
    -- Check client arguments.
    if type(color1) ~= "userdata" or color1.__type ~= "Color" or type(color2) ~= "userdata" or color2.__type ~= "Color" then return end
