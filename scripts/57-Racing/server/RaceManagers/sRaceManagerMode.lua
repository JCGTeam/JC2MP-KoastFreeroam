...
function RaceManagerMode:__init()
    RaceManagerBase.__init(self)
...
function RaceManagerMode:CreateRace()
    ...
    local args = {raceInfo = self:MarshalCurrentRace(), nextRaceInfo = self:MarshalNextRace()}
...
function RaceManagerMode:MarshalCurrentRace()
    return {currentPlayers = self.race.numPlayers, course = self.race.course:MarshalInfo(), collisions = self.race.vehicleCollisions}
end
...
function RaceManagerMode:UpdateNextRaceInfo()
    ...
    self.nextRaceInfo = {course = course, collisions = collisions, force = false}
end

function RaceManagerMode:MarshalNextRace()
    return {courseName = self.nextRaceInfo.course.name, collisions = self.nextRaceInfo.collisions}
end
...
function RaceManagerMode:UpdateVoteSkipInfo()
    ...
    local args = {skipVotes = skipVotes, skipVotesRequired = self:GetRequiredSkipVotes()}
...
function RaceManagerMode:ManagedPlayerJoin(player)
    ...
    local args = {className = "RaceManagerMode", nextRaceInfo = self:MarshalNextRace()}
...
function RaceManagerMode:RacerFinish(args)
    ...
    if self.raceInfo == nil or args.id ~= self.raceInfo.id then return end
...
function RaceManagerMode:PreTick(args)
    ...
    if self.raceInfo and self.raceInfo.raceEndTime and self.raceInfo.timer:GetSeconds() > self.raceInfo.raceEndTime then
        self.raceInfo = nil
        self.race:Terminate()
...
function RaceManagerMode:VoteSkip(vote, player)
    if type(vote) ~= "boolean" or self:HasPlayer(player) == false or self.raceInfo == nil or self.raceInfo.raceEndTime ~= nil then return end

    if vote == true then
        if table.find(self.raceInfo.skipVotes, player) == nil then
            table.insert(self.raceInfo.skipVotes, player)
            self:Message(player:GetName() .. " проголосовал за пропуск гонки")
        end
    else
        table.erase(self.raceInfo.skipVotes, player)
    end
...
function RaceManagerMode:RequestSpectate(unused, player)
    ...
    if self.race.playerIdToRacer[player:GetId()] then
        if self.race.numPlayers > 1 then
            ...
            self:Message(tostring(player) .. " перешел в режим наблюдения")

            self.race:RemovePlayer(player)
            self.race:AddSpectator(player)
        else
            player:SendChatMessage("Невозможно наблюдать; недостаточно игроков", Color.DarkGray)
...
function RaceManagerMode:AdminSkip(unused, player)
    if player:GetValue("isRaceAdmin") ~= true then return end

    self:Message(player:GetName() .. " принудительно пропустил текущую гонку")
...
function RaceManagerMode:AdminSetNextCourse(courseName, player)
    ...
    if course then
        self:Message(player:GetName() .. " изменил следующая карту на " .. course.name)
    else
        player:SendChatMessage(courseName .. " не найдено!", Color.Red)
    end
end
