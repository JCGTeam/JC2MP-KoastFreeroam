...
function CourseCheckpoint:Spawn()
    self.checkpoint = Checkpoint.Create {
        ...
        activation_box = Vector3(self.radius, self.radius, self.radius)
    }
...
function CourseCheckpoint:Enter(racer)
    if racer.hasFinished == false and self:GetIsValidVehicle(racer.player:GetVehicle()) and racer.targetCheckpoint ==
        self.index and racer.isRespawning == false then
