...
function Racer:__init(race, player)
    ...
    -- Send info to client.
    local args = {className = "Race", raceInfo = self.race.info}
...
function Racer:Update(racePosInfo)
    ...
    if self.player:GetState() == PlayerState.StuntPos and vehicle and vehicle:GetId() ~= self.assignedVehicleId then
...
function Racer:AdvanceCheckpoint(index)
    ...
    if self.hasFinished == false then
        self.race.state.racePosTracker[self.numCheckpointsHit][self.player:GetId()] = (self.targetCheckpointDistanceSqr)
    end
...
function Racer:Respawn()
    ...
    if self.player:GetHealth() == 0 then return end
    ...
    if checkpointIndex <= 0 then
        ...
        if #checkpoint.respawnPoints > 0 then
            ...
            for index, respawnPoint in ipairs(checkpoint.respawnPoints) do
                if respawnPoint.modelId == 0 or respawnPoint.modelId == self.assignedVehicleInfo.modelId then
        ...
        if usedRespawnPoint == false then
            ...
            if previousCheckpoint then
                ...
                if nextCheckpoint then
                    spawnDirection = (spawnDirection + (nextCheckpoint.position - checkpoint.position)) * 0.5
                end
            ...
            spawnAngle = Angle.FromVectors(Vector3.Forward, spawnDirection)
            spawnAngle.roll = 0
