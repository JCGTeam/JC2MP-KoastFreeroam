...
Course.LoadFromMap = function(map)
    ...
    while true do
        ...
        if course.type == "Circuit" and startingCheckpointItem.checkpoint == map.properties.firstCheckpoint then
            break
        end
    ...
    for objectIdSometimes, object in pairs(map.objects) do
        if object.type == "RaceVehicleInfo" then
            local vehicleInfo = {modelId = object.properties.modelId, templates = object.properties.templates, available = 0}
