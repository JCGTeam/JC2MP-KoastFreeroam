...
function Race:__init(args)
    ...
    if self.course.type == "Circuit" then
        self.numLaps = settings.numLapsFunc(self.numPlayers, #self.course.spawns, self.course.numLaps)
    end
    ...
    if self.overflowHandling == Race.OverflowHandling.StackSpawns and self.numPlayers > maxPlayers then
        if settings.debugLevel >= 2 then
            print(string.format("%i/%i", self.numPlayers, maxPlayers) .. " players, turning collisions off")
        end
    ...
    for index, player in ipairs(args.players) do
        self.info.playerIdToInfo[player:GetId()] = {name = player:GetName(), color = player:GetColor()}
    end
...
function Race:RemoveRacer(racer)
    if self.state.RacerLeave then self.state:RacerLeave(racer) end
...
function Race:RemoveSpectator(spectator)
    if self.state.SpectatorLeave then self.state:SpectatorLeave(spectator) end
...
function Race:Terminate()
    ...
    if self.world then self.world:Remove() self.world = nil end
...
function Race:RacerFinish(racer)
    ...
    racer.player:SetNetworkValue("RaceWinsCount", (racer.player:GetValue("RaceWinsCount") or 0) + 1)
    ...
    local args = {id = self.id, playerId = racer.playerId}
