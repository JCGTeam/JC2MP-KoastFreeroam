function HUD:DrawHUD()
    ...
    self:DrawLineHUD(Vector2.Zero, Vector2(0, 0.09))
    ...
    local DrawPitchLine = function(y, text, num)
        if num == 0 then
            self:DrawLineHUD(Vector2(-pitchLineX1 * 1.1, y), Vector2(-pitchLineX2 * 1.1, y), true)
            self:DrawLineHUD(Vector2(-pitchLineX1 * 1.1, y), Vector2(-pitchLineX1 * 1.1, y - 0.06), true)

            self:DrawLineHUD(Vector2(pitchLineX2 * 1.1, y), Vector2(pitchLineX1 * 1.1, y), true)
            self:DrawLineHUD(Vector2(pitchLineX1 * 1.1, y), Vector2(pitchLineX1 * 1.1, y - 0.06), true)
        elseif num > 0 then
            self:DrawLineHUD(Vector2(-pitchLineX1, y), Vector2(-pitchLineX2, y), true)
            self:DrawLineHUD(Vector2(pitchLineX2, y), Vector2(pitchLineX1, y), true)
        else
            self:DrawLineHUD(Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.04), y), Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.16), y), true)
            self:DrawLineHUD(Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.24), y), Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.36), y), true)
            self:DrawLineHUD(Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.44), y), Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.56), y), true)
            self:DrawLineHUD(Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.64), y), Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.76), y), true)
            self:DrawLineHUD(Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.84), y), Vector2(math.lerp(-pitchLineX1, -pitchLineX2, 0.96), y), true)
    ...
    -- Top.
    self:DrawLineHUD(Vector2(-altBoxSizeX, altBoxSizeY) + altBoxCenter, Vector2(altBoxSizeX, altBoxSizeY) + altBoxCenter)
    -- Bottom.
    self:DrawLineHUD(Vector2(-altBoxSizeX, -altBoxSizeY) + altBoxCenter, Vector2(altBoxSizeX, -altBoxSizeY) + altBoxCenter)
    -- Left.
    self:DrawLineHUD(Vector2(-altBoxSizeX, altBoxSizeY) + altBoxCenter, Vector2(-altBoxSizeX, -altBoxSizeY) + altBoxCenter)
    ...
    self:DrawLineHUD(Vector2(altBoxSizeX, altBoxSizeY) + altBoxCenter, Vector2(altBoxSizeX, -altBoxSizeY) + altBoxCenter)
    -- Text
    self:DrawTextHUD(string.format("%i", altitude), altBoxCenter + Vector2(altBoxSizeX * 1, 0.0), "right")
    ...
    -- Top.
    self:DrawLineHUD(Vector2(-speedBoxSizeX, speedBoxSizeY) + speedBoxCenter, Vector2(speedBoxSizeX, speedBoxSizeY) + speedBoxCenter)
    -- Bottom.
    self:DrawLineHUD(Vector2(-speedBoxSizeX, -speedBoxSizeY) + speedBoxCenter, Vector2(speedBoxSizeX, -speedBoxSizeY) + speedBoxCenter)
    -- Left.
    self:DrawLineHUD(Vector2(-speedBoxSizeX, speedBoxSizeY) + speedBoxCenter, Vector2(-speedBoxSizeX, -speedBoxSizeY) + speedBoxCenter)
    ...
    self:DrawTextHUD(speedString, speedBoxCenter + Vector2(speedBoxSizeX * 1, 0.0), "right")
    ...
    -- Arrow.
    self:DrawLineHUD(Vector2(0, compassY), Vector2(0, compassY) + Vector2(-0.05, -0.05))
    self:DrawLineHUD(Vector2(0, compassY), Vector2(0, compassY) + Vector2(0.05, -0.05))
    ...
    for n = from, to, 5 do
        if n >= leftValue and n <= rightValue then
            ...
            if n % 10 == 0 then
                ...
                self:DrawTextHUD(string.format("%i", n), Vector2(pos, compassY + textY), "center")
            else
                -- Small lines.
                lineHeight = lineHeight / 2
            end

            self:DrawLineHUD(Vector2(pos, compassY), Vector2(pos, compassY + lineHeight))
        end
    end
end