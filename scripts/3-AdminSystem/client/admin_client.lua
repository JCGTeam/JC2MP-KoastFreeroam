...
function Admin:__init()
    ...
    for _, model in ipairs({1, 2, 4, 7, 8, 9, 10, 11, 12, 13, 15, 18, 21, 22, 23, 26, 29, 31, 32, 33, 35, 36, 40, 41, 42, 43, 44, 46, 47, 48, 49, 52, 54, 55, 56, 60, 61, 63, 66, 70, 71, 72, 73, 74, 76, 77, 78, 79, 83, 84, 86, 87, 89, 90, 91, 5, 6, 16, 19, 25, 27, 28, 38, 45, 50, 69, 80, 88, 3, 14, 30, 34, 37, 39, 51, 57, 59, 62, 64, 65, 67, 81, 85, 75, 58, 82, 20, 53, 24}) do
        ...
        table.insert(self.vehicleList, name)
...
function Admin:LoadPanel()
    ...
    self.panel.main.playerAngle = GUI:Label("Угол: ?", Vector2(0.17, 0.32), Vector2(0.2, 0.03), self.panel.main.playersTab)
    self.panel.main.playerModel = GUI:Label("Персонаж: ?", Vector2(0.17, 0.35), Vector2(0.2, 0.03), self.panel.main.playersTab)
    ...
    self.panel.main.serverWeather = GUI:Label("Погода: ?", Vector2(0.005, 0.25), Vector2.Zero, self.panel.main.serverTab)
    ...
    GUI:Label("(0-2)", Vector2(0.21, 0.26), Vector2(0.03, 0.03), self.panel.main.serverTab)
    self.panel.main.serverTimeStep = GUI:Label("Скорость суток: ?", Vector2(0.005, 0.28), Vector2.Zero, self.panel.main.serverTab)
    ...
    GUI:Label("∞", Vector2(0.21, 0.29), Vector2(0.03, 0.03), self.panel.main.serverTab)
    ...
    self.panel.main.banDate = GUI:Label("Дата: ?", Vector2(0.17, 0.13), Vector2(0.2, 0.03), self.panel.main.bansTab)
    self.panel.main.banResponsible = GUI:Label("Забанил: ?", Vector2(0.17, 0.16), Vector2(0.2, 0.03), self.panel.main.bansTab)
    self.panel.main.banResponsibleSteam = GUI:Label("Админский Steam ID: ?", Vector2(0.17, 0.19), Vector2(0.2, 0.03), self.panel.main.bansTab)
    self.panel.main.banReasonScroll = GUI:ScrollControl(Vector2(0.17, 0.22), Vector2(0.2, 0.25), self.panel.main.bansTab)
    self.panel.main.banReason = GUI:Label("Причина: ?", Vector2.Zero, Vector2(0.2, 0.3), self.panel.main.banReasonScroll)
    ...
    self.panel.main.chatBottomBW = BaseWindow.Create(self.panel.main.adminchatTab)
    self.panel.main.chatBottomBW:SetDock(GwenPosition.Bottom)
    self.panel.main.chatBottomBW:SetHeight(35)

    self.panel.main.chatMessage = GUI:TextBox("", Vector2(0.0, 0.66), Vector2(0.38, 0.04), "text", self.panel.main.chatBottomBW)
    ...
    self.panel.main.clearMessage = GUI:Button("Очистить", Vector2(0.44, 0.66), Vector2(0.05, 0.04), self.panel.main.chatBottomBW, "adminchat.clearmessages")
    ...
    self.panel.main.sendMessage = GUI:Button(">", Vector2(0.385, 0.66), Vector2(0.04, 0.04), self.panel.main.chatBottomBW)
    ...
    self.panel.main.aclDataLabel = GUI:Label( "Имя группы: ?\n\nСоздатель: ?\n\nДата создания: ?\n\nОбъекты группы: ?", Vector2(0.205, 0.01), Vector2.Zero, self.panel.main.aclTab)
    ...
    self.panel.vehColor.window = GUI:Window("Цвет транспорта", Vector2.Zero, Vector2(0.3, 0.36)) GUI:Center(self.panel.vehColor.window)
...
function Admin:PlayerQuit(args)
    ...
    if players[pId] then
        self.panel.main.playersList:RemoveItem(players[pId])
...
function Admin:banPlayer()
    if IsValid(self.victim, false) then
        local reason = self.panel.ban.reasonCheck:GetChecked() and self.panel.ban.reasonEdit:GetText() or self.panel.ban.reasonsBox:GetSelectedItem():GetText()
...
function Admin:kickPlayer()
    if IsValid(self.victim, false) then
        local reason = self.panel.kick.reasonCheck:GetChecked() and self.panel.kick.reasonEdit:GetText() or self.panel.kick.reasonsBox:GetSelectedItem():GetText()
...
function Admin:warnPlayer()
    if IsValid(self.victim, false) then
        local reason = self.panel.warn.reasonCheck:GetChecked() and self.panel.warn.reasonEdit:GetText() or self.panel.warn.reasonsBox:GetSelectedItem():GetText()
...
function Admin:mutePlayer()
    if IsValid(self.victim, false) then
        local reason = self.panel.mute.reasonCheck:GetChecked() and self.panel.mute.reasonEdit:GetText() or self.panel.mute.reasonsBox:GetSelectedItem():GetText()
...
function Admin:spectate()
    ...
    if player then
        ...
        if self.victim ~= LocalPlayer then
            ...
            if self.updTimer then self.updTimer = nil end
...
function Admin:spectateRender()
    ...
    if self.victim and self.spVictimPos and self.spVictimAng and IsValid(self.victim, false) then
        ...
        Render:DrawShadowedText(Vector2(Render.Size.x / 2 - Render:GetTextWidth(spectate_txt .. "?", 20) / 2, Render.Size.y - 65), spectate_txt .. "?", Color.White, Color.Black, 20)
    end
end
...
function Admin:shout(args)
    if isTimer(self.shoutTimer) then killTimer(self.shoutTimer) end
...
function Admin:manualBan()
    ...
    if steamID ~= "" then
        local reason = self.panel.manualBan.reasonCheck:GetChecked() and self.panel.manualBan.reasonEdit:GetText() or self.panel.manualBan.reasonsBox:GetSelectedItem():GetText()
...
Events:Subscribe("ModuleLoad", function() admin = Admin() end)