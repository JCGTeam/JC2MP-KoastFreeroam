...
function Tetris:MoveCurrent(x, y)
    ...
    for _, v in pairs(self.current[self.currentRot]) do
        ...
        elseif y == 0 and self.gridNotCurrent[v.x + self.currentLoc.x + x][v.y + self.currentLoc.y] then
            self.moving = false
            return
        elseif (v.y + self.currentLoc.y + y) > 20 or
            self.gridNotCurrent[v.x + self.currentLoc.x + x][v.y + self.currentLoc.y + y] then
            canMove = false
        end
    end
...
function Tetris:FindMaxDropDistance()
    ...
    for _, v in pairs(self.current[self.currentRot]) do
        for y = 1, 20 do
            if (v.y + self.currentLoc.y + y) > 20 or
                self.gridNotCurrent[v.x + self.currentLoc.x][v.y + self.currentLoc.y + y] then
                if y < distance then
                    distance = y
                end
            end
...
function Tetris:GameOver()
    ...
    elseif totalScore > ((object:GetValue("S") or 0) * 0.6) and (object:GetValue("N") or "None") ~=
...
function Tetris:KeyDown(args)
    ...
    if self.inTetrisMode and (args.key == VirtualKey.Control) and not (self.settingsVisible or self.inGame) then
