...
function DailyTasks:GetNeededs(args, sender)
    Network:Send(sender, "NewNeededs", {
        completedtasksneeded = self.completedtasksneeded,
        fireworksneeded = self.fireworksneeded,
        flyingrecordneeded = self.flyingrecordneeded,
        tetrisrecordneeded = self.tetrisrecordneeded,
        driftrecordneeded = self.driftrecordneeded,
        tronwinsneeded = self.tronwinsneeded
    })
end
...
function DailyTasks:PlayerQuit(args)
    ...
    if args.player:GetValue("CompletedTasks") then cmd:Bind(2, args.player:GetValue("CompletedTasks")) end
    if args.player:GetValue("TronWins") then cmd:Bind(3, args.player:GetValue("TronWins")) end
    if args.player:GetValue("TetrisRecord") then cmd:Bind(4, args.player:GetValue("TetrisRecord")) end
    if args.player:GetValue("DriftRecord") then cmd:Bind(5, args.player:GetValue("DriftRecord")) end
    if args.player:GetValue("FlyingRecord") then cmd:Bind(6, args.player:GetValue("FlyingRecord")) end
    if args.player:GetValue("Bloozing") then cmd:Bind(7, args.player:GetValue("Bloozing")) end
    if args.player:GetValue("FireworksTossed") then cmd:Bind(8, args.player:GetValue("FireworksTossed")) end
    if args.player:GetValue("Prize") then cmd:Bind(9, args.player:GetValue("Prize")) end
    cmd:Execute()
end
