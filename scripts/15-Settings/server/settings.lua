...
function Settings:GetDBSettings( args )
	...
	args.player:SetNetworkValue( "ClockVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].clockvisible ) ) or true )
	args.player:SetNetworkValue( "ClockPendosFormat", #result > 0 and self:NumberToBoolean( tonumber( result[1].clockpendosformat ) ) or false )
	args.player:SetNetworkValue( "BestRecordVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].bestrecordsvisible ) ) or true )
	args.player:SetNetworkValue( "PassiveModeVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].passivemodevisible ) ) or true)
	args.player:SetNetworkValue( "JesusModeVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].jesusmodevisible ) ) or true )
	args.player:SetNetworkValue( "KillFeedVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].killfeedvisible ) )  or true )
	args.player:SetNetworkValue( "ChatTipsVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].chattipsvisible ) ) or true )
	args.player:SetNetworkValue( "ChatBackgroundVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].chatbackgroundvisible ) ) or false )
	args.player:SetNetworkValue( "PlayersMarkersVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].playersmarkersvisible ) ) or true )
	args.player:SetNetworkValue( "JobsMarkersVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].jobsmarkersvisible ) ) or true)
	args.player:SetNetworkValue( "CustomCrosshair", #result > 0 and self:NumberToBoolean( tonumber( result[1].customcrosshair ) ) or true )
	args.player:SetNetworkValue( "JetHUD", #result > 0 and self:NumberToBoolean( tonumber( result[1].jethud ) ) or false )
	args.player:SetNetworkValue( "LongerGrappleVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].longergrapplevisible ) ) or true )
	args.player:SetNetworkValue( "JobsVisible", #result > 0 and self:NumberToBoolean( tonumber( result[1].jobsvisible ) ) or true )
	args.player:SetNetworkValue( "LongerGrappleEnabled", #result > 0 and self:NumberToBoolean( tonumber( result[1].longergrappleenabled ) ) or true )
	args.player:SetNetworkValue( "VehicleEjectBlocker", #result > 0 and self:NumberToBoolean( tonumber( result[1].vehicleejectblocker ) ) or false )
	args.player:SetNetworkValue( "WingsuitEnabled", #result > 0 and self:NumberToBoolean( tonumber( result[1].wingsuitenabled ) ) or true )
	args.player:SetNetworkValue( "HydraulicsEnabled", #result > 0 and self:NumberToBoolean( tonumber( result[1].hydraulicsenabled ) ) or true )
end
