...
function BetterChat:__init()
    self.variations_ru = {"Неудачно", "Удачно"}
    self.variations_en = {"Unsuccessful", "Successfully"}
...
function BetterChat:Chat(args)
    ...
    elseif cmd_name == "try" then
        ...
        for p in Server:GetPlayers() do
            ...
            if dist < 50 then
                p:SendChatMessage(p:GetValue("Lang") == "EN" and "[Action] " or "[Действие] ", tagColor, pName .. " " .. tostring(table.concat(cmd_args, " ")), self.msgActionColor, " | ", self.msgColor, p:GetValue("Lang") == "EN" and table.randomvalue(self.variations_en) or table.randomvalue(self.variations_ru), self.msgActionColor, " | ", self.msgColor)
            end
        end
