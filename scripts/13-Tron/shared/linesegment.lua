...
function LineSegment:Intersects(segment) -- This is pretty accurate, but it does /not/ take angles into account; that's a bit beyond the scope of this anyway.
    ...
    if intersects then
        ...
        if collisionY >= segmentCollisionY and collisionY <= segmentCollisionY + segment.height or collisionY +
            self.height >= segmentCollisionY and collisionY + self.height <= segmentCollisionY + segment.height then
