...
function Tron:LocalPlayerExitVehicle(args)
    ...
    if self.state == GamemodeState.INPROGRESS and not self.collisionFired then
        Network:Send("Collision", {vehicle = args.vehicle, fell = true})
...
function Tron:PlayerNetworkValueChange(args)
    if args.value == true and args.key == "TronFiring" and args.player:InVehicle() and
        self.segments[args.player:GetVehicle():GetId()] then
    ...
    if self.state == GamemodeState.INPROGRESS then
        ...
        for k, segments in pairs(self.segments) do
            ...
            if IsValid(vehicle) then
                ...
                if (#segments == 0 or segments[#segments]:Length() > 10) and vehicle:GetDriver() and
                    vehicle:GetDriver():GetValue("TronFiring") then
        ...
        if LocalPlayer:InVehicle() then
            ...
            if self.timer:GetSeconds() > 5 and not self.collisionFired then
                Network:Send("Collision", {vehicle = vehicle})
                self.collisionFired = true
            end
        end
    end
