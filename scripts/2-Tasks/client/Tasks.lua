...
function Tasks:SharedObjectValueChange(args)
...
function Tasks:NetworkObjectValueChange(args)
    if args and args.object.__type ~= "LocalPlayer" then return end

    self.JobsVisibleValue = LocalPlayer:GetValue("JobsVisible")
    self.JobsMarkersVisibleValue = LocalPlayer:GetValue("JobsMarkersVisible")
end
...
function Tasks:DrawLocation2(k, v, dist, dir, jobDistance)
    ...
    t:Scale(textScale)
    t:Rotate(angle)
    t:Translate(-textBoxScaleDivided)
    ...
    if v.position:Distance(lpPos) <= 3.5 and not (job or lpVehicle) then
        ...
        if not self.HiddenHUDValue then
...
function Tasks:DrawTargetArrow(args)
    ...
    if shouldDraw and args.model then
        local transform = self.transform3
...
function Tasks:Render()
    ...
    if not self.JobsVisibleValue then return end
    if Game:GetState() ~= GUIState.Game then return end
    if LocalPlayer:GetWorld() ~= DefaultWorld then return end

    local gameAlpha = Game:GetSetting(4)

    if gameAlpha <= 1 or self.HiddenHUDValue then return end

    local jobsTable = self.jobsTable
    local locations = self.locations

    if jobsTable then
        local markersIsVisible = self.JobsMarkersVisibleValue and self.markers
    ...
    if job then
        ...
        local textShadow = Color(0, 0, 0, sett_alpha)
        local textSize = 15
        ...
        local jobColor = self.jobcolor
        local tagColor = Color(jobColor.r, jobColor.g, jobColor.b, sett_alpha)
        local defaultColor = Color(255, 255, 255, sett_alpha)

        Render:DrawShadowedText(textPos, target_txt, tagColor, textShadow, textSize)
...
function Tasks:GameRender()
    if not self.JobsVisibleValue or Game:GetState() ~= GUIState.Game or LocalPlayer:GetWorld() ~= DefaultWorld then return end
