...
function Pong:MouseMove(args)
    if active then
        if args.position.y >= mouse_last.y and (ping_pos + ping_height) < pong_table_height then
            ...
            if (ping_pos + ping_height) + (args.position.y - mouse_last.y) > pong_table_height then
                ping_pos = (pong_table_height - ping_height)
            end
        elseif args.position.y <= mouse_last.y and ping_pos > 0 then
            ping_pos = ping_pos - (mouse_last.y - args.position.y)

            if ping_pos - (mouse_last.y - args.position.y) < 0 then
                ping_pos = 0
            end
        end
...
function Pong:StartUp(args)
    ...
    elseif args.value == 2 then
        if not difficulty_level[args.cparams[2]] then
            Chat:Print("Неверная сложность!", Color(255, 0, 0))
            return false
        end
...
function Pong:HandleBallData()
    if (ball_pos.x) >= (pong_table_width - ping_width - 20) and (ball_pos.y + ball_height) >= ping_pos_opp and
        ball_pos.y <= (ping_pos_opp + ping_height) then
        ...
        if (ball_pos.x) > (pong_table_width - ping_width - 20) then ball_pos.x = pong_table_width - ping_width - 20 end
    elseif ball_pos.x <= ping_width + 10 and (ball_pos.y + ball_height) >= ping_pos and ball_pos.y <=
        (ping_pos + ping_height) then
        ball_speed.x = -ball_speed.x
