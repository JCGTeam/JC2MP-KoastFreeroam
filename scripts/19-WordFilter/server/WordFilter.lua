...
function WordFilter:EnableWF()
    if not self.PlayerChatEvent then self.PlayerChatEvent = Events:Subscribe("PlayerChat", self, self.PlayerChat) end
end

function WordFilter:DisableWF()
    if self.PlayerChatEvent then Events:Unsubscribe(self.PlayerChatEvent) self.PlayerChatEvent = nil end
end
