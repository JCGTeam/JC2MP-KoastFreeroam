...
function Bank:SendMoney(args, sender)
    if not (args.selectedplayer or args.money) then return end
...
function Bank:IsUpdated(player)
    return (self.updated[player:GetSteamId().id] == true)
end

function Bank:MarkUpdated(player)
    self.updated[player:GetSteamId().id] = true
end

function Bank:UnmarkUpdated(player)
    self.updated[player:GetSteamId().id] = false
end

function Bank:AddToQueue(player, money)
    if not IsValid(player) then return end

    self.money_queue[player:GetSteamId().id] = money
end
