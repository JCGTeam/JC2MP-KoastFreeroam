...
function CBoardServer:SyncPlayersData()
    Network:Broadcast("SyncPlayerData", {playersData = self:getPlayersDataList(), slotsNum = Config:GetValue("Server", "MaxPlayers")})
    return self
end

function CBoardServer:SyncPlayerData(player)
    Network:Send(player, "SyncPlayerData", {playersData = self:getPlayersDataList(), slotsNum = Config:GetValue("Server", "MaxPlayers")})
    return self
end
...
Events:Subscribe("ModuleLoad", function() CBoardServer = CBoardServer() end)