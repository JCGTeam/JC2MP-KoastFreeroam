...
function C4Controller:InputPoll()
    if not self.planted then
        ...
        local c4Count = LocalPlayer:GetValue("C4Count") or 0
        if c4Count and c4Count >= maxC4 then return end

        local actions = {Action.LookLeft, Action.LookUp, Action.LookRight, Action.LookDown, Action.MoveLeft, Action.MoveForward, Action.MoveRight, Action.MoveBackward}
...
function C4Controller:FireC4()
    if self.planted then
        ...
        if distance < (cameraPos.y < 200.5 and C4Controller.MaxPlantDistance / 2 or C4Controller.MaxPlantDistance) and distance > 1 then
            ...
            self.underhand = not (self.crouched or self.tossed) and distance > C4Controller.MaxPlantDistance - 2
            self.planted = false
...
function C4Controller:ApplyDummy(player)
    ...
    if table.find({AnimationState.LaSPlaceTriggeredExplosive, AnimationState.LaSUnderThrowGrenade, AnimationState.LaSThrowTriggeredExplosive}, state) then
        if self.anim then
            if not dummy then
                dummy = ClientStaticObject.Create({model = "wea35-tte.lod", position = Vector3(), angle = Angle()})
    ...
    elseif state == AnimationState.LaSWieldGrenade or player == LocalPlayer and self.detonationTimer:GetSeconds() <
        C4Controller.DetonationTimeout then
        if not dummy then
            dummy = ClientStaticObject.Create({model = "km05_02d.seq.blz/gp700_01-controller.lod", position = Vector3(), angle = Angle()})
